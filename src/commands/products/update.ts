...
export default function register(parent: Command): void {
  parent
    .command('update <id>')
    .description('Update a product\'s fields (partial update)')
    ...
    .action(async (id: string, cmdOpts: {
      ...
      await run(async () => {
        ...
        const input = {
          ...(cmdOpts.title !== undefined && { title: cmdOpts.title }),
        ...
        if (dryRun) {
          printDryRun('PATCH', `${baseUrl}/api/v1/products/${encodeURIComponent(id)}`, { product: input });
          return;
        }
